...
void VulkanEngine::draw()
{
	...
	// second
	VK_CHECK(vkWaitForFences(_device, 1, &get_current_frame()._renderFence, true, ONE_SEC));
	...
	//VK_CHECK(vkAcquireNextImageKHR(_device, _swapchain, ONE_SEC, get_current_frame()._swapchainSemaphore, nullptr, &swapchainImageIndex));
	VkResult e = vkAcquireNextImageKHR(_device, _swapchain, ONE_SEC, get_current_frame()._swapchainSemaphore, nullptr, &swapchainImageIndex);
	...
	_drawExtent.height = (uint32_t)std::min(_swapchainExtent.height, _drawImage.imageExtent.height) * renderScale;
	_drawExtent.width = (uint32_t)std::min(_swapchainExtent.width, _drawImage.imageExtent.width) * renderScale;
